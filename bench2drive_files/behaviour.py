...
def answer_behaviour_questions(self, ego_vehicle, other_vehicles, scene_data, current_measurement, scenario,
    ...
    ### extra question?
    # question_51 = "Identify the closest front vehicle (must have 'to the front' in relative position) with the smallest distance value (X in 'Xm to the front')" 
    question_51 = " Identify vehicles in the same lane as the ego vehicle by checking: (1) their speed direction relative to ego is within ±30° (0°=same direction), " +\
                " and (2) their lateral distance (left/right) is ≤2.5m. Then, from these same-lane vehicles, find the most dangerous preceding vehicle (must be in front of ego has 'Xm to the front' in relative position) by calculating TTC (Time To Collision = longitudinal distance / speed difference). " +\
                " Output in the following format: The most dangerous preceding vehicle id is X (if there is preceding vehicle, if there is no any preceding vehicle output: There is no preceding vehicle)" 
    answer_51 = f""
    self.add_qas_questions(qa_list=qas_conversation_behaviour,
                                qid=51,
                                chain=3,
